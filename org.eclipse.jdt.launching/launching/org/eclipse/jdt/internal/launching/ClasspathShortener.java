...
public class ClasspathShortener {
	 ...
	 * @param envp
	 *                                  array of strings, each element of which has environment variable settings in the format name=value, or null if
	...
	public ClasspathShortener(String os, String javaVersion, ILaunch launch, String[] cmdLine, int lastJavaArgumentIndex, File workingDir, String[] envp) {
		this.os = os;
		...
		this.processTempFilesDir = workingDir != null ? workingDir : new File(".").getAbsoluteFile(); //$NON-NLS-1$
	}
	 ...
	 *
	 * @return environment variable in the format name=value or null
	 */
	 ...
	 * {@link IJavaLaunchConfigurationConstants#ATTR_USE_CLASSPATH_ONLY_JAR}, a classpath-only jar is used (without asking confirmation).
	 *
	...
	protected boolean getLaunchConfigurationUseClasspathOnlyJarAttribute() throws CoreException {
		return launch.getLaunchConfiguration().getAttribute(IJavaLaunchConfigurationConstants.ATTR_USE_CLASSPATH_ONLY_JAR, false);
	}
	...
	protected int getMaxCommandLineLength() {
		...
		switch (os) {
			case Platform.OS_LINUX:
				...
				return ARG_MAX_LINUX - getEnvpSize() - 2048;
	...
	private void shortenClasspathUsingClasspathArgumentFile(int classpathArgumentIndex) throws CoreException {
		...
		replaceCmdLineArgs(classpathArgumentIndex - 1, 2, '@' + file.getAbsolutePath());
	...
	private void shortenModulePathUsingModulePathArgumentFile(int modulePathArgumentIndex) throws CoreException {
		...
		replaceCmdLineArgs(modulePathArgumentIndex - 1, 2, '@' + file.getAbsolutePath());
		addProcessTempFile(file);
	}
	...
	private void shortenClasspathUsingClasspathOnlyJar(int classpathArgumentIndex) throws CoreException {
		...
		replaceCmdLineArgs(classpathArgumentIndex, 1, classpathOnlyJar.getAbsolutePath());
	...
	private File createClasspathOnlyJar(String classpath) throws CoreException {
		try {
			...
			File jarFile = new File(processTempFilesDir, String.format(".%s-classpathOnly-%s.jar", getLaunchConfigurationName(), timeStamp)); //$NON-NLS-1$
	...
	private File createClassPathArgumentFile(String classpath) throws CoreException {
		try {
			String timeStamp = getLaunchTimeStamp();
			File classPathFile = new File(processTempFilesDir, String.format(".%s-classpath-arg-%s.txt", getLaunchConfigurationName(), timeStamp)); //$NON-NLS-1$
	...
	private File createModulePathArgumentFile(String modulePath) throws CoreException {
		try {
			String timeStamp = getLaunchTimeStamp();
			File classPathFile = new File(processTempFilesDir, String.format(".%s-module-path-arg-%s.txt", getLaunchConfigurationName(), timeStamp)); //$NON-NLS-1$
	...
	private void shortenClasspathUsingClasspathEnvVariable(int classpathArgumentIndex) {
		...
		String classpathEnvVar = "CLASSPATH=" + classpath; //$NON-NLS-1$
		...
		replaceCmdLineArgs(classpathArgumentIndex - 1, 2);
	}

	private void replaceCmdLineArgs(int index, int length, String... newArgs) {
